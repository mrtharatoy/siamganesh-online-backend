...
SUPABASE_URL/SUPABASE_KEY are empty strings in the test environment
(conftest.py), so the database/storage checks inside the route take
their documented "not configured" fallback paths without any network
call. This test locks in the full response shape from API_BASELINE.md
#17 so a future refactor (moving this into a `system` blueprint/service)
...
def test_system_status_shape_and_fallback_values_when_supabase_not_configured(app_module, client):
    ...
    with mock.patch.object(app_module, "CACHED_FILES", empty_cache), \
         mock.patch.object(app_module, "TOTAL_IMAGES_SIZE", empty_sizes):
