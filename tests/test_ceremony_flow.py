...
process_ceremony_flow() also sends Facebook messages and reads booking
rows from Supabase. We avoid real network calls without mocking away the
logic under test by relying on the *actual* early-return guards already
present in app.py:
...
The one boundary we do spy on (via mock.patch) is send_fb_action itself,
purely to assert *what* the code decided to send -- not to fake its
behavior.
"""
...
def test_returns_false_when_message_has_no_code_like_pattern(app_module):
    result = app_module.process_ceremony_flow(
...
def test_returns_true_and_reports_unknown_code_when_not_in_cache(app_module):
    ...
    with mock.patch.object(app_module, "send_fb_action", return_value=(True, "")) as mock_send:
        result = app_module.process_ceremony_flow(
...
def test_matches_code_prefix_against_cached_filenames(app_module):
    with mock.patch.dict(
        app_module.CACHED_FILES,
        {"mahabucha": {"150ab01": "150ab01.jpg"}},
    ), mock.patch.object(app_module, "send_fb_action", return_value=(True, "")) as mock_send:
        result = app_module.process_ceremony_flow(
